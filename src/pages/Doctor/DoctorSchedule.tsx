...
export default function DoctorSchedule({ doctorId }: DoctorScheduleProps) {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {getFilteredTimeSlots().length} khung giờ
              </p>
            </div>
            <button
  ...
  const renderTimeSlotCard = (slot: any, isSmall: boolean = false) => {
    const display = getTimeSlotDisplay(slot);
    const isExpired = isTimeSlotExpired(slot.specificDate, slot.endTime);
    
    return (
      <div 
        key={slot.id} 
        className={`p-${isSmall ? '3' : '4'} rounded-lg border-l-4 transition-all duration-200 ${
          isExpired
      ...
      >
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className={`font-mono ${isSmall ? 'text-sm' : 'text-lg'} font-${isSmall ? 'medium' : 'bold'} text-gray-800`}>
              {display.timeRange}
            </div>
            <div className={`${isSmall ? 'text-xs' : 'text-sm'} mt-1 font-medium ${display.statusColor}`}>
              {display.status}
            </div>
            {(display.isPast || isExpired) && (
              <div className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                <FaHistory size={10} />
                {isExpired ? 'Đã hết hạn' : 'Đã qua'}
              </div>
            )}
          </div>
          
          <div className={`flex ${isSmall ? 'items-center gap-1' : 'flex-col items-center gap-2'} ml-${isSmall ? '2' : '4'}`}>
            {!isExpired && !display.isPast && (
              <>
                <button
                ...
                >
                  {slot.isAvailable ? (
                    <FaToggleOn className="text-green-500" size={isSmall ? 16 : 20} />
                  ) : (
                    <FaToggleOff className="text-gray-400" size={isSmall ? 16 : 20} />
                  )}
                </button>
                <button
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-4">
        {weekDates.map((date, index) => {
          ...
          const daySlots = timeSlots.filter(slot => slot.specificDate === dateStr);
            ...
            >
              ...
              <div className="space-y-2 min-h-[200px]">
                {daySlots.length === 0 ? (
                  ...
                  daySlots.map(slot => renderTimeSlotCard(slot, true))
                )}
              </div>
            </div>
          );
              ...
              <div key={dateStr} className="border rounded-lg overflow-hidden">
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredSlots.map(slot => renderTimeSlotCard(slot, false))}
                  </div>
                </div>
              </div>
            );
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      ...
                      <strong> Khung giờ sẽ tự động bị vô hiệu hóa khi qua thời gian kết thúc.</strong>
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      {!loading && !error && renderScheduleContent()}
