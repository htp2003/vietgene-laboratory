...
import { DoctorCertificate } from '../../../api/doctorCertificates.api';
...
export default function DoctorCertificateManagement({ doctorId }: Props) {
  const {
    certificates,
    loading,
    error,
    ...
    updateCertificate,
    deleteCertificate,
  } = useDoctorCertificates(String(doctorId));
  ...
  const [form, setForm] = useState<Omit<DoctorCertificate, 'id' | 'created_at'>>({
    doctor_id: typeof doctorId === 'string' ? parseInt(doctorId, 10) : doctorId,
    certificate_name: '',
    certificate_number: '',
    issue_date: '',
    expiry_date: '',
          ...
          <tbody>
            {certificates.map((cert) => (
              <tr key={cert.id} className="hover:bg-gray-100">
                ...
                <td className="px-3 py-2 border">{cert.certificate_number}</td>
