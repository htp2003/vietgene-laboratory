...
// export default function DoctorTimeSlotManagement({ doctor }: DoctorTimeSlotManagementProps) {
...
//   const { slots, loading, error, addSlot, updateSlot, deleteSlot, toggleAvailable } = useDoctorTimeSlots(doctor.doctorId);
...
//           <div className="font-bold text-lg text-blue-800">{doctor.doctorCode}</div>
//           {doctor.phone && <div className="text-gray-600 ml-0 md:ml-6">{doctor.phone}</div>}
//           {doctor.email && <div className="text-gray-600 ml-0 md:ml-6">{doctor.doctorCode}</div>}
//           {doctor.isActive !== undefined && (
//             <div className={`ml-0 md:ml-6 px-2 py-1 rounded text-xs ${doctor.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{doctor.isActive ? 'Đang hoạt động' : 'Ngưng hoạt động'}</div>
//           )}
//         </div>
//       )}
