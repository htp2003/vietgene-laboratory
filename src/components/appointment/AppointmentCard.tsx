...
import { Calendar, Clock, MapPin, User, Phone, CheckCircle, XCircle, Eye, AlertCircle, Truck, Package, TestTube } from 'lucide-react';
...
const HomeTestStepper: React.FC<{ status: string }> = ({ status }) => {
  const steps = [
    ...
    { key: 'DeliveringKit', label: 'Giao kit', icon: Truck },
    { key: 'KitDelivered', label: 'Đã giao kit', icon: Package },
    { key: 'SampleReceived', label: 'Nhận mẫu', icon: TestTube },
    { key: 'Testing', label: 'Xét nghiệm', icon: AlertCircle },
...
const FacilityTestStepper: React.FC<{ status: string }> = ({ status }) => {
  const steps = [
    { key: 'Pending', label: 'Đặt lịch', icon: Calendar },
    { key: 'Confirmed', label: 'Check-in', icon: User },
    { key: 'SampleReceived', label: 'Nhận mẫu', icon: TestTube },
    { key: 'Testing', label: 'Xét nghiệm', icon: AlertCircle },
    { key: 'Completed', label: 'Hoàn thành', icon: CheckCircle }
  ];
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              ...
              {/* Only show doctor for facility appointments */}
            ...
            {appointment.status === 'Completed' && appointment.testResult && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-sm font-semibold text-green-800">Đã có kết quả xét nghiệm</h4>
                </div>
                ...
                <div className="mt-2 text-sm text-gray-700">
                  <span className="font-medium">Kết luận:</span> {appointment.testResult.conclusion}
                </div>
              </div>
            )}
          ...
          <div className="flex flex-col gap-2 ml-4">
            ...
            {appointment.status === 'DeliveringKit' && appointment.locationType === 'Tại nhà' && (
              <button
                onClick={() => alert(`Quản lý kit cho lịch hẹn ${appointment.id}`)}
